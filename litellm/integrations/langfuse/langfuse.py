...
class LangFuseLogger:
    ...
    def _build_emitted_metadata(
        ...
        """Return the allowlisted metadata, plus caller strings and litellm enrichments."""
        assembled: Final = {
    ...
    def _log_langfuse_v2(
        ...
        try:
            ...
            if debug is True or (isinstance(debug, str) and debug.lower() == "true"):
                # scalars only: the UserAPIKeyAuth object and the nested credential
                # dicts are the non-scalars, and this flag exists to show what the
                # caller sent rather than what the allowlist kept
            ...
            hidden_params: Final = (
                standard_logging_object.get("hidden_params", _NO_METADATA) if standard_logging_object else _NO_METADATA
            )
            ...
            candidate_enrichments: Final = (
                ...
                ("hidden_params", filter_exceptions_from_params(hidden_params), True),
            ...
            if self._supports_tags():
                if "cache_hit" in kwargs and kwargs["cache_hit"] is None:
                    # shared model_call_details: later callbacks read this key and
                    # record None instead of False without the normalization
